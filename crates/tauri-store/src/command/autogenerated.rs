...
where
  R: Runtime,
{
  let store_collection = app.store_collection();
  ids
    .iter()
    .for_each(|id| store_collection.allow_save(id));
}
  ...
  R: Runtime,
{
  ...
  ids
    .iter()
    .for_each(|id| store_collection.allow_sync(id));
}
  ...
  R: Runtime,
{
  ...
  ids
    .iter()
    .for_each(|id| store_collection.deny_save(id));
}
  ...
  R: Runtime,
{
  ...
  ids
    .iter()
    .for_each(|id| store_collection.deny_sync(id));
}
...
where
  R: Runtime,
{
  app.store_collection().destroy(id)
}
...
where
  R: Runtime,
{
  app.store_collection().raw_state(id)
}
